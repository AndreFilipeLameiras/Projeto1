...
class ListaCombustivelFragment: Fragment(), LoaderManager.LoaderCallbacks<Cursor> {
    ...
    override fun onCreateLoader(id: Int, args: Bundle?): Loader<Cursor> {
        return CursorLoader(
            requireContext(),
            ContentProviderCarros.ENDERECO_COMBUSTIVEIS,
            TabelaBDCombustivel.TODAS_COLUNAS,
            null,
            null,
            TabelaBDCombustivel.CAMPO_NOME
        )
    }
    ...
    override fun onLoadFinished(loader: Loader<Cursor>, data: Cursor?) {
        adapterCombustiveis!!.cursor = data
    }
    ...
    fun processaOpcaoMenu(item: MenuItem) : Boolean =
        when(item.itemId) {
            R.id.action_inserir -> {
                findNavController().navigate(R.id.action_listaCombustivelFragment_to_inserirCombustivelFragment)
                true
            }
