...
class ListaTracaoFragment : Fragment(), LoaderManager.LoaderCallbacks<Cursor>{
    ...
    override fun onCreateLoader(id: Int, args: Bundle?): Loader<Cursor> {
        return CursorLoader(
            requireContext(),
            ContentProviderCarros.ENDERECO_TRACOES,
            TabelaBDTracao.TODAS_COLUNAS,
            null,
            null,
            TabelaBDTracao.CAMPO_NOME
        )
    }
    ...
    fun processaOpcaoMenu(item: MenuItem): Boolean =
        when(item.itemId){
            R.id.action_inserir -> {
                ...
                (activity as MainActivity).atualizaTitulo(R.string.inserir_tracao_label)
                true
            }
            R.id.action_alterar -> {
                val acao = ListaTracaoFragmentDirections.actionListaTracaoFragmentToEditarTracaoFragment(tracaoSelecionada!!)
                findNavController().navigate(acao)
                (activity as MainActivity).atualizaTitulo(R.string.alterar_tracao_label)
                true
            }
