...
class TabelaBDMotorizacoes (db: SQLiteDatabase) : TabelaBD(db, NOME){
    override fun cria(){
        db.execSQL("CREATE TABLE $nome (${BaseColumns._ID} INTEGER PRIMARY KEY AUTOINCREMENT, $CAMPO_POTENCIA TEXT NOT NULL, $CAMPO_CONSUMO DOUBLE NOT NULL, $CAMPO_EMISSOES DOUBLE NOT NULL , $CAMPO_TRANSMISSOES_ID INTEGER NOT NULL,$CAMPO_TRACAO_ID INTEGER NOT NULL, $CAMPO_COMBUSTIVEL_ID INTEGER NOT NULL, FOREIGN KEY ($CAMPO_TRANSMISSOES_ID) REFERENCES ${TabelaBDTransmissoes.NOME},  FOREIGN KEY ($CAMPO_TRACAO_ID) REFERENCES ${TabelaBDTracao.NOME}, FOREIGN KEY ($CAMPO_COMBUSTIVEL_ID) REFERENCES ${TabelaBDCombustivel.NOME})")
